...
#define GET_PTR_FROM_SELF(p) QAxWidget *p = qobject_cast<QAxWidget *>(Qt4xHb::getQObjectPointerFromSelfItem())

HB_FUNC_STATIC(QAXWIDGET_NEW)
{
  if (ISBETWEEN(0, 2) && ISQWIDGETORNIL(1) && ISNUMORNIL(2)) {
    // QAxWidget(QWidget *parent = 0, Qt::WindowFlags f = 0)
    QAxWidget *obj = new QAxWidget(OPQWIDGET(1, 0), HB_ISNIL(2) ? static_cast<Qt::WindowFlags>(0) : PQT_WINDOWFLAGS(2));
    ...
    QAxWidget *obj =
        new QAxWidget(PQSTRING(1), OPQWIDGET(2, 0), HB_ISNIL(3) ? static_cast<Qt::WindowFlags>(0) : PQT_WINDOWFLAGS(3));
    Qt4xHb::returnNewObject(obj, false);
...
HB_FUNC_STATIC(QAXWIDGET_DYNAMICCALL)
{
  if (ISBETWEEN(1, 9) && HB_ISCHAR(1) && ISQVARIANTORNIL(2) && ISQVARIANTORNIL(3) && ISQVARIANTORNIL(4) &&
    ...
    if (obj != NULL) {
      RQVARIANT(obj->dynamicCall(PCONSTCHAR(1), HB_ISNIL(2) ? QVariant() : *static_cast<QVariant *>(Qt4xHb::itemGetPtr(2)),
