...
{
  QNetworkAccessManager_release_codeblocks();
}
void SlotsQNetworkAccessManager::authenticationRequired ( QNetworkReply * reply, QAuthenticator * authenticator )
{
  ...
  for (int i = 0; i < list1.size(); ++i)
  {
    ...
    {
      if( ( (QString) list2.at(i) == (QString) "authenticationRequired(QNetworkReply*,QAuthenticator*)" ) && ( (bool) list4.at(i) == true ) )
      {
        ...
        hb_vmEvalBlockV( (PHB_ITEM) list3.at(i), 3, psender, preply, pauthenticator );
...
void SlotsQNetworkAccessManager::finished ( QNetworkReply * reply )
{
  ...
  for (int i = 0; i < list1.size(); ++i)
  {
    if( (QObject*) list1.at(i) == (QObject*) object )
    {
      if( ( (QString) list2.at(i) == (QString) "finished(QNetworkReply*)" ) && ( (bool) list4.at(i) == true ) )
      {
        PHB_ITEM psender = hb_itemPutPtr( NULL, (QObject *) object );
        PHB_ITEM preply = hb_itemPutPtr( NULL, (QNetworkReply *) reply );
...
void SlotsQNetworkAccessManager::networkAccessibleChanged ( QNetworkAccessManager::NetworkAccessibility accessible )
{
  ...
  for (int i = 0; i < list1.size(); ++i)
  {
    if( (QObject*) list1.at(i) == (QObject*) object )
    {
      if( ( (QString) list2.at(i) == (QString) "networkAccessibleChanged(QNetworkAccessManager::NetworkAccessibility)" ) && ( (bool) list4.at(i) == true ) )
      {
        PHB_ITEM psender = hb_itemPutPtr( NULL, (QObject *) object );
        PHB_ITEM paccessible = hb_itemPutNI( NULL, (int) accessible );
        hb_vmEvalBlockV( (PHB_ITEM) list3.at(i), 2, psender, paccessible );
        hb_itemRelease( psender );
...
void SlotsQNetworkAccessManager::proxyAuthenticationRequired ( const QNetworkProxy & proxy, QAuthenticator * authenticator )
{
  ...
  for (int i = 0; i < list1.size(); ++i)
  {
    if( (QObject*) list1.at(i) == (QObject*) object )
    {
      if( ( (QString) list2.at(i) == (QString) "proxyAuthenticationRequired(QNetworkProxy,QAuthenticator*)" ) && ( (bool) list4.at(i) == true ) )
      {
        PHB_ITEM psender = hb_itemPutPtr( NULL, (QObject *) object );
        PHB_ITEM pproxy = hb_itemPutPtr( NULL, (QNetworkProxy *) &proxy );
        PHB_ITEM pauthenticator = hb_itemPutPtr( NULL, (QAuthenticator *) authenticator );
        hb_vmEvalBlockV( (PHB_ITEM) list3.at(i), 3, psender, pproxy, pauthenticator );
        hb_itemRelease( psender );
...
void SlotsQNetworkAccessManager::sslErrors ( QNetworkReply * reply, const QList<QSslError> & errors )
{
  ...
  for (int i = 0; i < list1.size(); ++i)
  {
    if( (QObject*) list1.at(i) == (QObject*) object )
    {
      if( ( (QString) list2.at(i) == (QString) "sslErrors(QNetworkReply*,QList<QSslError>)" ) && ( (bool) list4.at(i) == true ) )
      {
        PHB_ITEM psender = hb_itemPutPtr( NULL, (QObject *) object );
        PHB_ITEM preply = hb_itemPutPtr( NULL, (QNetworkReply *) reply );
        PHB_DYNS pDynSym;
        ...
        hb_vmEvalBlockV( (PHB_ITEM) list3.at(i), 3, psender, preply, perrors );
        hb_itemRelease( psender );
        hb_itemRelease( preply );
...
{
  QObject* object = (QObject*) hb_itemGetPtr( hb_objSendMsg( hb_stackSelfItem(), "POINTER", 0 ) );
  QString signal = "sslErrors(QNetworkReply*,QList<QSslError>)";
  if( s == NULL )
  {
    s = new SlotsQNetworkAccessManager(QCoreApplication::instance());
  }
  ...
  if( hb_pcount() == 1 )
  {
    ...
    if( found == -1 )
    {
      ret = object->connect( object, SIGNAL(sslErrors(QNetworkReply*,QList<QSslError>)), s, SLOT(sslErrors(QNetworkReply*,QList<QSslError>)) );
      if( ret ) // se conectado, adiciona
      {
        // procura por posição livre
        i = s->list4.indexOf( false );
  ...
  {
    ret = object->disconnect(object, SIGNAL(sslErrors(QNetworkReply*,QList<QSslError>)), s, SLOT(sslErrors(QNetworkReply*,QList<QSslError>)) );
    if( ret == true )
    {
      for (int i = 0; i < s->list1.size(); ++i)
      {
