...
HB_FUNC_STATIC(QABSTRACTSOCKET_NEW)
{
  if (ISNUMPAR(2) && HB_ISNUM(1) && ISQOBJECT(2)) {
    QAbstractSocket *obj = new QAbstractSocket(PQABSTRACTSOCKET_SOCKETTYPE(1), PQOBJECT(2));
...
HB_FUNC_STATIC(QABSTRACTSOCKET_SOCKETOPTION)
{
  ...
  if (obj != NULL) {
    ...
    if (ISNUMPAR(1) && HB_ISNUM(1)) {
#endif
      QVariant *ptr = new QVariant(obj->socketOption(PQABSTRACTSOCKET_SOCKETOPTION(1)));
      RQVARIANT(ptr);
#ifndef QT4XHB_DONT_CHECK_PARAMETERS
    } else {
      hb_errRT_BASE(EG_ARG, 3012, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS);
