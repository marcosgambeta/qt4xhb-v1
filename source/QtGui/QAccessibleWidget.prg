...
HB_FUNC_STATIC(QACCESSIBLEWIDGET_NEW)
{
  if (ISBETWEEN(1, 3) && ISQWIDGET(1) && ISNUMORNIL(2) && ISCHARORNIL(3)) {
    QAccessibleWidget *obj = new QAccessibleWidget(
        PQWIDGET(1), HB_ISNIL(2) ? static_cast<QAccessible::Role>(QAccessible::Client) : PQACCESSIBLE_ROLE(2),
        OPQSTRING(3, QString()));
...
#endif
      RQSTRING(obj->actionText(PINT(1), PQACCESSIBLE_TEXT(2), PINT(3)));
#ifndef QT4XHB_DONT_CHECK_PARAMETERS
    } else {
      hb_errRT_BASE(EG_ARG, 3012, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS);
    }
...
HB_FUNC_STATIC(QACCESSIBLEWIDGET_TEXT)
{
  ...
  if (obj != NULL) {
    ...
    if (ISNUMPAR(2) && HB_ISNUM(1) && HB_ISNUM(2)) {
#endif
      RQSTRING(obj->text(PQACCESSIBLE_TEXT(1), PINT(2)));
#ifndef QT4XHB_DONT_CHECK_PARAMETERS
    } else {
      hb_errRT_BASE(EG_ARG, 3012, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS);
    }
