...
#define GET_PTR_FROM_SELF(p) QGraphicsItem *p = static_cast<QGraphicsItem *>(Qt4xHb::itemGetPtrStackSelfItem())
...
HB_FUNC_STATIC(QGRAPHICSITEM_ENSUREVISIBLE)
{
  if (ISBETWEEN(0, 3) && (ISQRECTF(1) || HB_ISNIL(1)) && ISNUMORNIL(2) && ISNUMORNIL(3)) {
    ...
    if (obj != NULL) {
      obj->ensureVisible(HB_ISNIL(1) ? QRectF() : *static_cast<QRectF *>(Qt4xHb::itemGetPtr(1)), OPINT(2, 50),
                         OPINT(3, 50));
    }
...
#endif
      obj->setCacheMode((QGraphicsItem::CacheMode)hb_parni(1),
  ...
  if (obj != NULL) {
    ...
    if (ISNUMPAR(1) && HB_ISARRAY(1)) {
      ...
      for (int i1 = 0; i1 < nLen1; i1++) {
        par1 << static_cast<QGraphicsTransform *>(
    ...
    if (ISNUMPAR(0)) {
      ...
      if (pDynSym != NULL) {
        ...
        for (int i = 0; i < count; i++) {
          ...
          PHB_ITEM pObject = hb_itemNew(NULL);
          hb_itemCopy(pObject, hb_stackReturnItem());
          PHB_ITEM pItem = hb_itemPutPtr(NULL, static_cast<QGraphicsTransform *>(list[i]));
          hb_objSendMsg(pObject, "_POINTER", 1, pItem);
          hb_itemRelease(pItem);
          hb_arrayAddForward(pArray, pObject);
          hb_itemRelease(pObject);
