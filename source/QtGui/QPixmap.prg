...
HB_FUNC_STATIC(QPIXMAP_NEW)
{
  if (ISNUMPAR(0)) {
    ...
    QPixmap *obj = new QPixmap(PQSTRING(1), OPCONSTCHAR(2, 0),
...
HB_FUNC_STATIC(QPIXMAP_SCALED)
{
  if (ISBETWEEN(1, 3) && ISQSIZE(1) && ISNUMORNIL(2) && ISNUMORNIL(3)) {
    ...
    if (obj != NULL) {
      QPixmap *ptr = new QPixmap(obj->scaled(*PQSIZE(1),
    ...
    if (obj != NULL) {
      QPixmap *ptr = new QPixmap(obj->scaled(PINT(1), PINT(2),
...
#endif
    QPixmap *ptr =
        new QPixmap(QPixmap::grabWindow(static_cast<WId>(hb_parptr(1)), OPINT(2, 0), OPINT(3, 0), OPINT(4, -1), OPINT(5, -1)));
    Qt4xHb::createReturnClass(ptr, "QPIXMAP", true);
#ifndef QT4XHB_DONT_CHECK_PARAMETERS
  } else {
    hb_errRT_BASE(EG_ARG, 3012, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS);
