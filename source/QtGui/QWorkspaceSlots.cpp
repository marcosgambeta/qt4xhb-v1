...
static SlotsQWorkspace * s = NULL;

SlotsQWorkspace::SlotsQWorkspace(QObject *parent) : QObject(parent)
{
}
SlotsQWorkspace::~SlotsQWorkspace()
{
  QWorkspace_release_codeblocks();
}
void SlotsQWorkspace::windowActivated( QWidget * w )
{
  ...
  for (int i = 0; i < list1.size(); ++i)
  {
    ...
    {
      if( ( (QString) list2.at(i) == (QString) "windowActivated(QWidget*)" ) && ( (bool) list4.at(i) == true ) )
      {
        PHB_ITEM psender = hb_itemPutPtr( NULL, (QObject *) object );
        PHB_ITEM pw = hb_itemPutPtr( NULL, (QWidget *) w );
        hb_vmEvalBlockV( (PHB_ITEM) list3.at(i), 2, psender, pw );
...
HB_FUNC( QWORKSPACE_ONWINDOWACTIVATED )
{
  ...
  QString signal = "windowActivated(QWidget*)";
  if( s == NULL )
  {
    s = new SlotsQWorkspace(QCoreApplication::instance());
  }
  bool ret = false;
  if( hb_pcount() == 1 )
  {
    ...
    if( found == -1 )
    {
      ret = object->connect( object, SIGNAL(windowActivated(QWidget*)), s, SLOT(windowActivated(QWidget*)) );
  ...
  {
    ret = object->disconnect(object, SIGNAL(windowActivated(QWidget*)), s, SLOT(windowActivated(QWidget*)) );
    if( ret == true )
    {
