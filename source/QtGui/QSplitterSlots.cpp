...
void SlotsQSplitter::splitterMoved ( int pos, int index )
{
  ...
  for (int i = 0; i < list1.size(); ++i)
  {
    ...
    {
      if( ( (QString) list2.at(i) == (QString) "splitterMoved(int,int)" ) && ( (bool) list4.at(i) == true ) )
      {
        PHB_ITEM psender = hb_itemPutPtr( NULL, (QObject *) object );
        PHB_ITEM ppos = hb_itemPutNI( NULL, pos );
        PHB_ITEM pindex = hb_itemPutNI( NULL, index );
        hb_vmEvalBlockV( (PHB_ITEM) list3.at(i), 3, psender, ppos, pindex );
