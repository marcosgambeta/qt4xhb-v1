  ...
  if (ISNUMPAR(0)) {
    ...
    QRegion *obj =
        new QRegion(PINT(1), PINT(2), PINT(3), PINT(4),
                    HB_ISNIL(5) ? static_cast<QRegion::RegionType>(QRegion::Rectangle) : PQREGION_REGIONTYPE(5));
    Qt4xHb::returnNewObject(obj, true);
  } else if (ISBETWEEN(1, 2) && ISQPOLYGON(1) && ISNUMORNIL(2)) {
    // QRegion(const QPolygon &a, Qt::FillRule fillRule = Qt::OddEvenFill)
    QRegion *obj =
        new QRegion(*PQPOLYGON(1), HB_ISNIL(2) ? static_cast<Qt::FillRule>(Qt::OddEvenFill) : PQT_FILLRULE(2));
    Qt4xHb::returnNewObject(obj, true);
...
HB_FUNC_STATIC(QREGION_INTERSECTED)
{
  if (ISNUMPAR(1) && ISQREGION(1)) {
    ...
    if (obj != NULL) {
      RQREGION(obj->intersected(*PQREGION(1)));
    }
  } else if (ISNUMPAR(1) && ISQRECT(1)) {
    ...
    if (obj != NULL) {
      RQREGION(obj->intersected(*PQRECT(1)));
    }
  } else {
    hb_errRT_BASE(EG_ARG, 3012, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS);
  }
