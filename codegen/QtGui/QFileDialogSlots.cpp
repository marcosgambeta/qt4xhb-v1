...
{
  QFileDialog_release_codeblocks();
}
...
void SlotsQFileDialog::filesSelected ( const QStringList & selected )
{
  ...
  for (int i = 0; i < list1.size(); ++i)
  {
    if( (QObject*) list1.at(i) == (QObject*) object )
    {
      if( ( (QString) list2.at(i) == (QString) "filesSelected(QStringList)" ) && ( (bool) list4.at(i) == true ) )
      {
        PHB_ITEM psender = hb_itemPutPtr( NULL, (QObject *) object );
        PHB_ITEM pselected = hb_itemPutPtr( NULL, (QStringList *) &selected );
        hb_vmEvalBlockV( (PHB_ITEM) list3.at(i), 2, psender, pselected );
        hb_itemRelease( psender );
