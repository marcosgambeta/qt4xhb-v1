...
{
  QPlainTextEdit_release_codeblocks();
}
...
void SlotsQPlainTextEdit::updateRequest ( const QRect & rect, int dy )
{
  ...
  for (int i = 0; i < list1.size(); ++i)
  {
    if( (QObject*) list1.at(i) == (QObject*) object )
    {
      if( ( (QString) list2.at(i) == (QString) "updateRequest(QRect,int)" ) && ( (bool) list4.at(i) == true ) )
      {
        PHB_ITEM psender = hb_itemPutPtr( NULL, (QObject *) object );
        PHB_ITEM prect = hb_itemPutPtr( NULL, (QRect *) &rect );
        PHB_ITEM pdy = hb_itemPutNI( NULL, dy );
        hb_vmEvalBlockV( (PHB_ITEM) list3.at(i), 3, psender, prect, pdy );
        hb_itemRelease( psender );
