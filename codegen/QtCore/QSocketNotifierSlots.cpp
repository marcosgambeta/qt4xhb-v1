...
static SlotsQSocketNotifier * s = NULL;

SlotsQSocketNotifier::SlotsQSocketNotifier(QObject *parent) : QObject(parent)
{
}
SlotsQSocketNotifier::~SlotsQSocketNotifier()
{
  QSocketNotifier_release_codeblocks();
}
void SlotsQSocketNotifier::activated(int socket)
{
  ...
  for (int i = 0; i < list1.size(); ++i)
  {
    ...
    {
      if( ( (QString) list2.at(i) == (QString) "activated(int)" ) && ( (bool) list4.at(i) == true ) )
      {
        PHB_ITEM psender = hb_itemPutPtr( NULL, (QObject *) object );
        PHB_ITEM psocket = hb_itemPutNI( NULL, socket );
        hb_vmEvalBlockV( (PHB_ITEM) list3.at(i), 2, psender, psocket );
...
HB_FUNC( QSOCKETNOTIFIER_ONACTIVATED )
{
  ...
  if( s == NULL )
  {
    s = new SlotsQSocketNotifier(QCoreApplication::instance());
  }
  bool ret = false;
  if( hb_pcount() == 1 )
  {
